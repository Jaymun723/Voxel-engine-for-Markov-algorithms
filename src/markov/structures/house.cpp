...
bool BuildGroundRule::applies_to(const Chunk &C, const vec3 &pos) const {
  if (pos.x == 0 || pos.x == C.BLOCK_CHUNK_SIZE.x - 1 || pos.y == 0 ||
      pos.y == C.BLOCK_CHUNK_SIZE.y - 1) {
    return false; // Don't build on the edges of the chunk
  }
  vec3 directions[8] = {vec3(1, 0, 0),   vec3(-1, 0, 0), vec3(0, 1, 0),
  ...
  for (const vec3 &dir : directions) {
    ...
    if (C.m_pBlocks[(int)new_pos.x][(int)new_pos.y][(int)new_pos.z]
            .block_type == BlockType_Plank_Tmp) {
      return true;
    }
  }
  return false;
...
void BuildGroundRule::apply(Chunk &C, const vec3 &pos) const {
  C.m_pBlocks[(int)pos.x][(int)pos.y][(int)pos.z].block_type =
      BlockType_Plank_Tmp;
}
...
bool MakeMiddleDefRule::applies_to(const Chunk &C, const vec3 &pos) const {
  if (C.m_pBlocks[(int)pos.x][(int)pos.y][(int)pos.z].block_type !=
      BlockType_Plank_Tmp) {
    return false;
  }
  vec3 directions[8] = {vec3(1, 0, 0),   vec3(-1, 0, 0), vec3(0, 1, 0),
...
bool ElevateRule::applies_to(const Chunk &C, const vec3 &pos) const {
  ...
  if (pos.z > 0 &&
      (C.m_pBlocks[(int)pos.x][(int)pos.y][(int)pos.z - 1].block_type ==
           BlockType_Plank_Tmp ||
       C.m_pBlocks[(int)pos.x][(int)pos.y][(int)pos.z - 1].block_type ==
           BlockType_Plank)) {
    return true;
...
void ElevateRule::apply(Chunk &C, const vec3 &pos) const {
  ...
  C.m_pBlocks[x][y][z].block_type =
      BlockType_Plank_Tmp; // Otherwise, just make a plank
}
...
bool BuildRoofRule::applies_to(const Chunk &C, const vec3 &pos) const {
  ...
  for (const vec3 &dir : directions) {
    ...
    if (C.m_pBlocks[(int)new_pos.x][(int)new_pos.y][(int)new_pos.z]
                .block_type != BlockType_Plank_Tmp &&
        C.m_pBlocks[(int)new_pos.x][(int)new_pos.y][(int)new_pos.z]
                .block_type != BlockType_Plank) {
      return false;
    }
  }
  return true;
...
void BuildRoofRule::apply(Chunk &C, const vec3 &pos) const {
  if (pos.z + 1 <= C.BLOCK_CHUNK_SIZE.z) {
    C.m_pBlocks[(int)pos.x][(int)pos.y][(int)pos.z + 1].block_type =
        BlockType_Plank_Tmp; // Set the current block to Plank
  }
}
...
bool BuildFlatRoofRule::applies_to(const Chunk &C, const vec3 &pos) const {
  if (pos.z >= C.BLOCK_CHUNK_SIZE.z - 1) {
    return C.m_pBlocks[(int)pos.x][(int)pos.y][(int)pos.z].block_type !=
           BlockType_Empty; // Don't build a roof if we are at the top of the
                            // chunk
  }
  if (C.m_pBlocks[(int)pos.x][(int)pos.y][(int)pos.z + 1].block_type !=
      BlockType_Empty) {
    return false;
  }

  vec3 directions[4] = {vec3(1, 0, 0), vec3(-1, 0, 0), vec3(0, 1, 0),
                        vec3(0, -1, 0)};
  int count = 0;
  for (const vec3 &dir : directions) {
    vec3 new_pos = pos + dir;
...
void BuildFlatRoofRule::apply(Chunk &C, const vec3 &pos) const {
  C.m_pBlocks[(int)pos.x][(int)pos.y][(int)pos.z].block_type =
      BlockType_Flat_Roof; // Set the current block to Plank
}
...
void MakeExtDefRule::apply(Chunk &C, const vec3 &pos) const {
  C.m_pBlocks[(int)pos.x][(int)pos.y][(int)pos.z].block_type =
      BlockType_Plank; // Change the temporary plank to normal plank
}
...
ExtendedMR RemovePlankTmp =
    ExtendedMR({std::make_shared<RemovePlankTmpRule>()});
...
void AppearNewGroundRule::apply(Chunk &C, const vec3 &pos) const {
  ...
  C.m_pBlocks[x][y][z].block_type =
      BlockType_Plank_Tmp; // Change the flat roof to temporary grass
}

ExtendedMR AppearNewGround =
    ExtendedMR({std::make_shared<AppearNewGroundRule>()});
...
ExtendedMR CleanSingleBlocks =
    ExtendedMR({std::make_shared<CleanSingleBlocksRule>()});
...
bool BuildPillarsRule::applies_to(const Chunk &C, const vec3 &pos) const {
  ...
  int count = 0;
  vec3 directions[4] = {vec3(1, 0, 1), vec3(-1, 0, 1), vec3(0, 1, 1),
                        vec3(0, -1, 1)};
  for (const vec3 &dir : directions) {
    vec3 new_pos = pos + dir;
    if (new_pos.x < 0 || new_pos.x >= C.BLOCK_CHUNK_SIZE.x || new_pos.y < 0 ||
    ...
    if (C.m_pBlocks[(int)new_pos.x][(int)new_pos.y][(int)new_pos.z]
                .block_type == BlockType_Plank ||
        C.m_pBlocks[(int)new_pos.x][(int)new_pos.y][(int)new_pos.z]
                .block_type == BlockType_Flat_Roof) {
      ++count;
    }
  }
  return count == 2;
